...
def deduce_base_branch_name() -> str:
    ...
    raise ValueError(
        "Could not deduce the base branch name. Please specify it manually."
    )
...
def create_and_checkout_branch(
    new_branch_name: str, base_branch_name: str | None = None
) -> None:
    ...
    try:
        subprocess.run(
            ["git", "checkout", new_branch_name], check=True
